    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (response.status >= 500) {
          ...
          console.warn(`Discord server error ${response.status}, retrying in ${waitMs}ms (attempt ${attempt}/${maxRetries})`);
          await this.sleep(waitMs);
          continue;
        }
        ...
        if (attempt < maxRetries) {
          ...
          console.warn(`Discord network error, retrying in ${waitMs}ms (attempt ${attempt}/${maxRetries}):`, error);
          await this.sleep(waitMs);
          continue;
        }
