...
Python에서는 LangGraph가 이 문제를 풀었다. StateGraph에 Node와 Edge를 선언하고, Checkpointer로 상태를 스냅샷하고, interrupt로 HITL을 건다. 잘 만들어졌다. 근데 TypeScript에는 그에 대응하는 게 없었다 — LangGraph.js는 있지만, Python API의 포팅이지 TS 네이티브가 아니다.

Mastra는 이 gap에서 시작한 프로젝트다.
...
| 메모리 | 외부 통합 (LangMem 등) | 3계층 내장 (Working/Observational/Semantic) |
| 도구 프로토콜 | 외부 통합 | MCP 일급 지원 |
| DevTools | LangSmith (유료 SaaS) | Mastra Studio (로컬, OSS) |
...
하지만 TS 스택에서 프로덕션 AI 앱을 만드는 팀이라면, 이제 선택지가 생겼다. Vercel AI SDK만으로는 너무 low-level이고, LangGraph.js는 Python 포팅이라 TS 생태계와 이질적이고, 처음부터 만들기엔 인프라 비용이 너무 크다. Mastra는 정확히 이 gap에 있다.
