...
| Composer | Free-text submit, slash commands, in-flight steering | Editor stays open during loading; Enter steers by default; `/stop` interrupts explicitly | State machine tests; manual TTY smoke |
| Streaming | Progress, tool calls, text deltas, done metadata | Progress is an ambient one-line status and disappears once content streams; tool calls do not overlap answer text | Progress renderer tests; existing stream parser tests; manual live query |
...
| Cancellation | ESC, `/stop`, and Ctrl+C during loading | In-flight stream aborts; state records cancellation; terminal exits cleanly | Manual live query |
...
1. Run `bun run dev:dwkim` from the workspace root, or `bun run --filter dwkim dev`.
...
5. Start a long query, type a steering instruction, and press `Enter`.
6. Start another long query and press `ESC` or submit `/stop`.
7. Resize terminal narrower and submit one more short query.
8. Exit with `Ctrl+C`; cursor should return to the next shell line.
