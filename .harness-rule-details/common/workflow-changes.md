...
4. **Update State**: Mark stage as "PENDING" in `harness-state.md`
...
5. **Update State**: Mark stage as "SKIPPED" in `harness-state.md`
...
3. **If Restart Chosen**:
   ...
   - Mark stage as "IN PROGRESS" in `harness-state.md`
...
4. **If Confirmed**:
   ...
   - Reset all affected stages in `harness-state.md`
...
3. **Update State**: Ensure `harness-state.md` reflects current status
...
**On Resume**:
1. **Detect Existing Project**: Check for `harness-state.md`
...
2. **Update All Tracking**: Keep `harness-state.md`, plan files, and `audit.md` in sync
