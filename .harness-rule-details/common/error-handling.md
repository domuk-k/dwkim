...
**Error**: Existing `harness-state.md` is corrupted
...
**Scenario**: `harness-state.md` is corrupted or inconsistent

**Recovery Steps**:
1. Create backup: `harness-state.md.backup`
...
3. Reset stage status in `harness-state.md`
...
3. Mark stage as "SKIPPED" in `harness-state.md`
...
- **Solution**: 
  ...
  2. Check if stage was marked complete in harness-state.md
...
**Error**: harness-state.md shows stage complete but artifacts don't exist
- **Cause**: State file updated but artifact generation failed
- **Solution**:
  1. Mark stage as incomplete in harness-state.md
...
1. **Always validate state**: Check harness-state.md matches actual artifacts
