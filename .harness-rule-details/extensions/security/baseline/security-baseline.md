...
## Overview
These security rules are MANDATORY cross-cutting constraints that apply across all Harness phases. They are not optional guidance — they are hard constraints that stages MUST enforce when generating questions, producing design artifacts, generating code, and presenting completion messages.
...
4. The finding MUST be logged in `harness-docs/audit.md` with the SECURITY rule ID, description, and stage context
...
These rules are cross-cutting constraints that apply to every Harness stage. At each stage:
